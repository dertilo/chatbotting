...
def is_new_offer(prev_offer_name, system_act):
    return (
        ...
        and prev_offer_name != system_act.params[0].value
    )
...
class AgendaBasedUS(object):
    ...
    def receive_input(self, system_acts: List[DialogueAct]):
        ...
        new_offers = list(
            filter(lambda x: is_new_offer(self.prev_offer_name, x), system_acts)
        )
        if len(new_offers) > 0:
    ...
    def _handle_inform_and_offer(self, system_act: DialogueAct):
        ...
        if self._check_that_all_slots_do_meet_constraints_and_update_agenda(items):
    ...
    def _check_that_all_slots_do_meet_constraints_and_update_agenda(
